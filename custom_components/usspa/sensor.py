...
SENSOR_KEYS = ["ActTemp","LastConn","ActDateTime","ProductionDate","SpaInstallDate","EnergyResetDateTime","TmpEnergyResetDateTime",
               "HeatingSpeed","Pump3ActSpeed","Pump3ReqSpeed","OzoneRuntime","Pump3Runtime","Filtration","Model","SwVer","FanStart","FanStop"]
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities):
    ...
    for key in SENSOR_KEYS:
        if key in coordinator.data:
            ents.append(USSPASensor(coordinator, entry, client, key))
...
class USSPASensor(CoordinatorEntity, SensorEntity):
    def __init__(self, coordinator, entry: ConfigEntry, client, key: str) -> None:
        ...
        self._key = key
        meta = get_meta(key)
        self._attr_unique_id = f"{entry.entry_id}_sensor_{key.lower()}"
        if meta["name"]:
    ...
    def native_value(self) -> Any:
        return (self.coordinator.data.get(self._key) or {}).get("value")
